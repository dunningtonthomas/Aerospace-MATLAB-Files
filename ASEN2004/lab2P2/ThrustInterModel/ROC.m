function [dX] = ROC(t,state,const,wind,thrustVec,timeVec)
...
%           thrustVec = vector of thrust values from static test
%           timeVec = corresponding time for the thrustVec
...
%Calculating thrust
[timeThrust, ind] = min(abs(t - timeVec));

if(ind >= length(timeVec)) %Base case, thrust is over
    thrust = 0;
    
elseif(t < timeVec(1)) %First case
    thrust = thrustVec(1);

elseif(t > timeVec(ind)) %Interpolate with next value
    diff = thrustVec(ind + 1) - thrustVec(ind);
    slope = diff / (timeVec(ind + 1) - timeVec(ind));
    deltaT = t - timeVec(ind);
    thrust = thrustVec(ind) + slope * deltaT;

elseif(t < timeVec(ind)) %Interpolate with previous value
    diff = thrustVec(ind) - thrustVec(ind - 1);
    slope = diff / (timeVec(ind) - timeVec(ind - 1));
    deltaT = timeVec(ind) - t;
    thrust = thrustVec(ind - 1) + slope * deltaT;
