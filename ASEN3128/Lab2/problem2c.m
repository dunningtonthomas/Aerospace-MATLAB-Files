...
%Force and moment coefficients for aerodynamics
mu = 2e-6;
nu = 1e-3;
...
odeFunc = @(t,var)AircraftEOM(t,var,g,mass,aeroForceCoeff,aeroMomentCoeff,forces_steady_hover,moment_steady_hover);
...
%% Plotting
figure();
plot(time, finalState(:,1));

ylabel('Position x');
xlabel('Time');

figure();
plot(time, finalState(:,2));
