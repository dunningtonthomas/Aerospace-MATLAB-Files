...
shortEigen = eigenLon(:,3);
shortEigen = shortEigen / (shortEigen(4));
shortEigen = shortEigen * (2*pi/180); %Scaling so initial pitch is 2 degrees
shortEigenReal = real(shortEigen); %Order [u,w,q,theta,x,z]
addInitialShort = [shortEigenReal(5); 0; shortEigenReal(6); 0; shortEigenReal(4); 0; shortEigenReal(1); 0; shortEigenReal(2); 0; shortEigenReal(3); 0];

%Adding the phugoid eigenvector to the initial trim state to get the
%initial condition
aircraft_state0 = trim_state + addInitialShort;
...
%% Short Period mode
system = ss(A,B,C,D);
[deltaX,TOUT2,~] = initial(system, shortEigenReal,tfinal);
...
for i = 1:length(deltaX(:,1))
     ...
     YOUT2(i,1) = TOUT2(i) * Va_trim; %Hardcoding in the change in x with the trim velocity
end
