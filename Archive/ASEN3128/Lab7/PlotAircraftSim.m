...
if (~isempty(control_surfaces))
    ...
    subplot(411);
    plot(TOUT, control_surfaces(:,1)*(180/pi),col, 'linewidth', 2);hold on;
    title('Control Surfaces v Time');   
    ylabel('\delta_e [deg]')    

    subplot(412);
    plot(TOUT, control_surfaces(:,2)*(180/pi),col, 'linewidth', 2);hold on;
    ylabel('\delta_a [deg]')      
 
    subplot(413);
    plot(TOUT, control_surfaces(:,3)*(180/pi),col, 'linewidth', 2);hold on;
    ylabel('\delta_r [deg]')       
...
subplot(311);
h7= plot(TOUT, wind_angles(1,:) ,col, 'linewidth', 2);hold on;
title('Wind Angles v Time');   
ylabel('V_a [m/s]')    

subplot(312);
plot(TOUT, (180/pi)*wind_angles(2,:),col, 'linewidth', 2);hold on;
 ylabel('\beta [deg]')    
 
subplot(313);
plot(TOUT, (180/pi)*wind_angles(3,:),col, 'linewidth', 2);hold on;
ylabel('\alpha [deg]')    
xlabel('time [sec]');
...
figure(8);
subplot(211)
plot(TOUT, chi, col, 'linewidth', 2);hold on;
ylabel('Course angle \chi [deg]')   

subplot(212)
plot(TOUT, gamma, col, 'linewidth', 2);hold on;
ylabel('Flight path angle \gamma [deg]')  

figure(9);
plot(aircraft_state(:,1),aircraft_state(:,2), 'linewidth', 2);
xlabel('X position (m)');
ylabel('Y position (m)');
