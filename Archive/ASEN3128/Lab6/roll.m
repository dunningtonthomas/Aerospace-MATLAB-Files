...
rollEigen = eigenLat(:,3);
rollEigen = rollEigen / (rollEigen(4));
rollEigen = rollEigen * (2*pi/180); %Scaling so initial pitch is 2 degrees
rollReal = real(rollEigen); %Order [v,p,r,phi,psi,y]
addRoll = [0; rollReal(6); 0; rollReal(4); 0; rollReal(5); 0; rollReal(1); 0; rollReal(2); 0; rollReal(3)];

%Adding the phugoid eigenvector to the initial trim state to get the
%initial condition
aircraft_state0 = trim_state + addRoll;
...
%% Phugoid mode
system = ss(A,B,C,D);
[deltaX,TOUT2,~] = initial(system, rollReal,tfinal);
