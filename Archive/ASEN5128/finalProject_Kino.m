...
desired_waypoints = getWaypoints("Data/waypoints_kino_2", h_trim);

% Apply a spline fit to the waypoints
x_spline = linspace(min(desired_waypoints(:,1)), max(desired_waypoints(:,1)), 1000);
spline_waypoints = spline(desired_waypoints(:,1), desired_waypoints(:,2), x_spline);
lookahead_dist = 75;
goal_point = desired_waypoints(end,:)';
goal_tolerance = 20;

waypoint_index = 1;
waypoint_tolerance = 75;
waypoint = desired_waypoints(waypoint_index, :)';
...
for i=1:n_ind
    ...
    % If close enough to the waypoint, move on to the next waypoint
    if(norm(current_pos(1:2) - waypoint(1:2)) < waypoint_tolerance)
        waypoint_index = waypoint_index + 1;
    end

    % Get desired waypoint 
    if(waypoint_index > length(desired_waypoints(:,1)))
        break; % Reached the final waypoint
    end
    waypoint = desired_waypoints(waypoint_index, :)';

    % Get desired velocity
    vel_d = waypointGuidance(current_pos, waypoint, current_vel);
    control_objectives = getControlObjectives(vel_d, waypoint);


    % Get desired velocity
    % [vel_d, waypoint] = splineGuidance(current_pos, desired_waypoints, current_vel, lookahead_dist);
    % control_objectives = getControlObjectives(vel_d, waypoint);
    % 
...
%save("DubinsPath.mat", "aircraft_array", "time_iter", "control_array", "desired_waypoints");
...
% Plot the waypoints
plot(desired_waypoints(:,1), desired_waypoints(:,2), 'r', 'linewidth',  2, 'linestyle', '--', 'DisplayName', 'Dubins Path');

% Add circles around the waypoints using viscircles
% for i = 1:size(desired_waypoints, 1)
%     viscircles(desired_waypoints(i, 1:2), waypoint_tolerance, 'LineStyle', '--', 'EdgeColor', 'r', 'LineWidth', 1.5);
% end

% Formatting the plot
xlabel('East (m)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('North (m)', 'FontSize', 12, 'FontWeight', 'bold');
title('Autopilot Dubins Path Tracking', 'FontSize', 14, 'FontWeight', 'bold');
axis equal;
grid on;
...
lineWidth = 2;
customColor = [0.2, 0.4, 0.8]; % A pleasing shade of blue
...
sgtitle('Aircraft Control Surfaces', 'FontSize', titleFontSize + 2, 'FontWeight', 'bold', 'Interpreter', 'latex');
